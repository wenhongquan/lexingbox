...
class HYInfiniteScrollView: UIView, UIScrollViewDelegate{
    ...
    var _animationDuration: NSTimeInterval = 5.0      //滚动的时间间隔
    ...
    var sliderPageControl: HYPageControlView {
        get{
            if _sliderPageControl == nil {
                let pageRect = CGRectMake(self.bounds.size.width/2 - 23, self.bounds.size.height - 5 - 7, 46, 7);
    ...
    func reloadActivityItem(items: NSArray){
        ...
        for item in items {
            var imageView: UIbrandImageView = item as! UIbrandImageView
            imageView.frame = CGRectMake(0,0,self.bounds.size.width,self.scrollView.frame.size.height)
        ...
        if(items.count>0){
            ...
            _sliderPageControl?.pageCount = self.totalPageCount!
            
            _sliderPageControl?.setPageNumber(self.totalPageCount!)
        }
